...
describe('createJobAnalysisTrigger — durable delivery recovery', () => {
  ...
  it('cancels a scheduled ACK retry when the trigger stops', async () => {
    vi.useFakeTimers()
    try {
      const markConsumed = vi.fn().mockRejectedValue(new Error('database busy'))
      const deps = createDeps({
        findPersistedDelivery: vi.fn().mockReturnValue({
          messageId: 'persisted-message-1',
          outcome: 'succeeded'
  ...
  it('handles settlement dependency failures without an unhandled rejection', async () => {
    const findPersistedDelivery = vi
      .fn()
      .mockReturnValueOnce({ messageId: 'persisted-message-1', outcome: 'succeeded' })
      .mockImplementationOnce(() => {
        throw new Error('projection unavailable')
      })
    const deps = createDeps({ findPersistedDelivery })
    const trigger = createJobAnalysisTrigger(deps)
    ...
    expect(deps.log).toHaveBeenCalledWith(
      ...
      expect.stringContaining('projection unavailable')
    )
  })
...
describe('createJobAnalysisTrigger — application queue admission', () => {
  it('hands off each job once even while application admission remains pending', async () => {
    const sendPrompt = vi.fn(() => new Promise<undefined>(() => undefined))
