...
type ConfiguredShellSettings = Pick<StoredSettings, 'localShellRuntime' | 'wslSelection'>
...
const resolveConfiguredShellRuntimeBinding = (
  ...
  if (settings.localShellRuntime === 'wsl2-bash') {
    const selection = settings.wslSelection
    if (!selection) throw new Error('The selected WSL2 Shell profile is unavailable.')
