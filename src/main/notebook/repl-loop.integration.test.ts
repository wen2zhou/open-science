...
describe('repl_loop local RPC transport', () => {
  ...
  it('routes the published host.skills composer without exposing unreleased resource methods', async () => {
    let received: { method?: string; params?: Record<string, unknown> } = {}
    const server = createServer((request, response) => {
      let body = ''
      request.on('data', (chunk) => (body += chunk))
    ...
    try {
      ...
      const unreleased = await send(
        'return [typeof host.skills.resource, typeof host.skills.stage]'
      )
      expect(unreleased.error).toBeNull()
      expect(JSON.parse(unreleased.result ?? '[]')).toEqual(['undefined', 'undefined'])
    } finally {
      child.kill()
      await new Promise<void>((resolve, reject) =>
        server.close((error) => (error ? reject(error) : resolve()))
