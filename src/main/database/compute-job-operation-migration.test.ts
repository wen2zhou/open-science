...
describe('Compute Job operation migration', () => {
  it('replays an unledgered operation suffix without rewriting ComputeJob', async () => {
    ...
    await client.$executeRawUnsafe(
      `DELETE FROM "_open_science_migrations" WHERE "id" IN ('0023_compute_job_operation', '0024_compute_job_file_evidence', '0025_managed_file_version_foundation', '0026_compute_job_cleanup')`
    )
    ...
    ).resolves.toEqual([{ id: '0026_compute_job_cleanup' }])
  })
