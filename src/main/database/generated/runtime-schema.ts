...
const RUNTIME_SCHEMA_TABLE_DDLS = [
  ...
  `CREATE TABLE IF NOT EXISTS "ComputeJobCancellation" (
    ...
    CONSTRAINT "ComputeJobCancellation_state_check" CHECK ("state" IN ('requested', 'claimed', 'retry_wait', 'confirmed', 'superseded')),
    CONSTRAINT "ComputeJobCancellation_revision_check" CHECK ("revision" >= 1),
    CONSTRAINT "ComputeJobCancellation_attempt_check" CHECK ("attempt" >= 0),
    CONSTRAINT "ComputeJobCancellation_lease_check" CHECK (("state" = 'claimed' AND "leaseToken" IS NOT NULL AND "leaseExpiresAt" IS NOT NULL) OR ("state" <> 'claimed' AND "leaseToken" IS NULL AND "leaseExpiresAt" IS NULL)),
    CONSTRAINT "ComputeJobCancellation_terminal_time_check" CHECK (("state" = 'confirmed' AND "confirmedAt" IS NOT NULL AND "supersededAt" IS NULL) OR ("state" = 'superseded' AND "supersededAt" IS NOT NULL AND "confirmedAt" IS NULL) OR ("state" IN ('requested', 'claimed', 'retry_wait') AND "confirmedAt" IS NULL AND "supersededAt" IS NULL))
);`,
