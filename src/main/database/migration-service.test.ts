...
describe('application database migrations', () => {
  ...
  it('keeps a recovery snapshot when final verification rejects current schema drift', async () => {
    ...
    await expect(
      ...
      code: 'database_validation_failed',
      migrationId: '0026_compute_job_cleanup'
    })
  ...
  it('creates recovery snapshots while retaining only the newest two for a ledger database', async () => {
    ...
    expect(backupEvents).toEqual([
      ...
      expect.objectContaining({
        migrationId: '0026_compute_job_cleanup',
        path: `${databasePath}.before-0026_compute_job_cleanup.backup`,
        reused: false
      })
    ])
    ...
    await expect(
      access(`${databasePath}.before-0026_compute_job_cleanup.backup`)
    ).resolves.toBeUndefined()
    await expect(
  ...
  it('retains only the two newest restorable snapshots after adopting a legacy database', async () => {
    ...
    await expect(
      ...
      'open-science.db.before-0026_compute_job_cleanup.backup'
    ])
    await expect(access(backupPath)).rejects.toMatchObject({ code: 'ENOENT' })
    ...
    const backupClient = new PrismaClient({
      datasources: {
        db: {
          url: `file:${`${databasePath}.before-0025_managed_file_version_foundation.backup`.replaceAll('\\', '/')}`
        }
      }
    })
    try {
  ...
  it('upgrades the real upstream ledger and repairs VisionEvidence after the managed rebuild', async () => {
    ...
    await expect(migrateApplicationDatabase(client)).resolves.toEqual({
      adoptedLegacy: false,
      applied: ['0025_managed_file_version_foundation', '0026_compute_job_cleanup'],
