...
type ComputeJobResultDeliveryAdapterDeps = Readonly<{
  ...
  acknowledge(context: TerminalAgentResultDeliveryContext): Promise<AgentResultFollowUpDelivery>
}>
...
class ComputeJobResultDeliveryAdapter {
  ...
  observeResult(job: JobSummary): Promise<AgentResultFollowUpDelivery> {
    // Provider terminality precedes harvest. Until harvest is durable, getJobResult intentionally
    // exposes only a partial projection and must not suppress delivery of the eventual full result.
    ...
    return this.serialize(job.job_id, async () => {
      // A direct result read is also a durable admission repair: even if the initial broadcast was
      // lost, recording the terminal fact and its observation in one serialized chain prevents a
      // later recovery pass from creating a redundant continuation.
      await this.deps.repository.registerComputeJob(this.registration(job))
      disposition = await this.deps.acknowledge(computeJobDeliveryContext(job))
    }).then(() => disposition)
  }
