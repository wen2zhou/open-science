...
const harness = (): {
  ...
  const acknowledge = vi.fn(async () => 'suppressed' as const)
  return {
    adapter: new ComputeJobResultDeliveryAdapter({ repository, enqueue, acknowledge }),
    repository,
    enqueue,
    acknowledge
  }
...
describe('ComputeJobResultDeliveryAdapter', () => {
  ...
  it('serializes a fast terminal notification behind durable nonterminal registration', async () => {
    ...
    const enqueue = vi.fn(async () => undefined)
    const adapter = new ComputeJobResultDeliveryAdapter({
      repository,
      enqueue,
      acknowledge: vi.fn(async () => 'suppressed' as const)
    })
  ...
  it('repairs admission and acknowledges a terminal result read without enqueueing it', async () => {
    const { adapter, repository, enqueue, acknowledge } = harness()
    const result = job({
      status: 'success',
      harvested_at: 10,
      notified_at: 11,
    ...
    await expect(adapter.observeResult(result)).resolves.toBe('suppressed')

    expect(repository.registerComputeJob).toHaveBeenCalledWith(
      expect.objectContaining({ jobId: 'job-1', sessionId: 'session-1' })
    )
    expect(acknowledge).toHaveBeenCalledWith(
      expect.objectContaining({ sourceKind: 'compute-job', jobId: 'job-1' })
    )
    expect(enqueue).not.toHaveBeenCalled()
  })

  it('does not acknowledge a terminal result until harvest makes its projection final', async () => {
    const { adapter, repository, acknowledge } = harness()

    await adapter.observeResult(job({ status: 'success', harvested_at: undefined }))

    expect(repository.registerComputeJob).not.toHaveBeenCalled()
    expect(acknowledge).not.toHaveBeenCalled()
  })

  it('acknowledges a final dispatch error that has no harvest phase', async () => {
    const { adapter, repository, acknowledge } = harness()

    await adapter.observeResult(
      job({ status: 'error', harvested_at: undefined, error_code: 'dispatch_failed' })
    )

    expect(repository.registerComputeJob).toHaveBeenCalledOnce()
    expect(acknowledge).toHaveBeenCalledWith(
      expect.objectContaining({ jobId: 'job-1', terminalStatus: 'error' })
    )
  })
