...
const assertStableOutcomeIdentity = (
  row: PrismaAgentResultDelivery,
  context: TerminalAgentResultDeliveryContext
): void => {
  const sourceKind = 'jobId' in context ? 'compute-job' : 'local-run'
  const sourceId = 'jobId' in context ? context.jobId : context.runId
  const agentFrameId = 'jobId' in context ? undefined : context.agentFrameId
  if (
    row.sourceKind !== sourceKind ||
    row.sourceId !== sourceId ||
    row.projectId !== context.projectId ||
    row.sessionId !== context.sessionId ||
    row.executionType !== context.executionType ||
    row.agentFrameId !== (agentFrameId ?? null)
  ) {
    throw new Error(`Conflicting delivery ownership for ${sourceKind} ${sourceId}.`)
  }
  if (row.terminalStatus !== 'waiting-result' && row.terminalStatus !== context.terminalStatus) {
    throw new Error(`Conflicting terminal status for ${sourceKind} ${sourceId}.`)
  }
}
...
class AgentResultDeliveryRepository {
  ...
  async recordTerminalOutcome(
    ...
    if (context.sourceKind === 'compute-job') {
      await client.agentResultDelivery.updateMany({
        where: {
          sourceKind: 'compute-job',
      ...
      assertStableOutcomeIdentity(row, context)
      return toDelivery(row)
    }
    const id = `local-run:${context.runId}`
    // Ensure the durable row exists before the conditional terminal transition. A separate
    // background-admission callback may race this path; upsert makes both orderings monotonic and
    // avoids the former findUnique(null) -> create unique-key window.
    const admitted = await client.agentResultDelivery.upsert({
      where: { sourceKind_sourceId: { sourceKind: 'local-run', sourceId: context.runId } },
      create: {
        id,
        sourceKind: 'local-run',
    ...
    assertStableOutcomeIdentity(admitted, context)
    await client.agentResultDelivery.updateMany({
      where: {
        sourceKind: 'local-run',
        sourceId: context.runId,
        projectId: context.projectId,
        sessionId: context.sessionId,
        agentFrameId: context.agentFrameId ?? null,
        executionType: context.executionType,
        state: 'waiting-result'
      },
      data: {
    ...
    if (!row) throw new Error(`Local Run delivery ${id} disappeared after admission.`)
    assertStableOutcomeIdentity(row, context)
    return toDelivery(row)
  }
  ...
  async listProjectVisible(projectId: string, limit = 200): Promise<AgentResultDelivery[]> {
    ...
    const rows = await client.agentResultDelivery.findMany({
      where: {
        ...
        dismissedAt: null,
        state: {
          in: ['waiting-result', 'pending', 'claimed', 'dispatching', 'needs-attention']
        }
      },
  ...
  async recoverExpiredClaims(now = Date.now()): Promise<number> {
    ...
    const recovered = await client.agentResultDelivery.updateMany({
      where: {
        ...
        claimExpiresAt: { lte: new Date(now) }
      },
  ...
  async markObserved(id: string, sessionId: string, observedAt = Date.now()): Promise<boolean> {
    const client = await this.getClient()
    const updated = await client.agentResultDelivery.updateMany({
      where: {
        id,
      ...
      data: {
        ...
        consumedAt: new Date(observedAt)
      }
    })
    return updated.count === 1
  }
