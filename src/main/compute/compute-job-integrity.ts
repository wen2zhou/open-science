...
const remoteHandleIsComplete = (row: IntegrityRow): boolean => {
  if (!row.remoteHandle) return true
  try {
    const handle = JSON.parse(row.remoteHandle) as Record<string, unknown> | null
    const workdir = row.remoteWorkdir
    return Boolean(
      handle &&
...
export const classifyComputeJobIntegrity = (row: IntegrityRow): ComputeJobIntegrityIssue[] => {
  ...
  if (
    ...
    issues.push(issue(row, { code: 'notified-before-terminal', disposition: 'needs-attention' }))
  }
