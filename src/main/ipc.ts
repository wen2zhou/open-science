...
import { SlurmDriver } from './compute/slurm-driver'
...
const registerIpcHandlers = async ({
  ...
  // Register the Direct SSH and Slurm drivers once (design.md §4.2). The dispatcher/poller resolve
  // them per job via the registry by the driver snapshotted at submit time (design.md §4.1).
  sharedComputeDriverRegistry.register(new DirectDriver({ runner: sshRunner }))
  sharedComputeDriverRegistry.register(new SlurmDriver({ runner: sshRunner }))
