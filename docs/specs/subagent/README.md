...
S1与S2均已通过各自production-composed release gate并达到`certified`；S2真实desktop journey覆盖inactive-branch Stop、partial failure、Send gate恢复与失败项重试。S5的Interface、durable Owner、三framework Adapter与root scheduler证据达到`conformant`，Main→child→Main、branch/restart与两child/真实user prompt journeys可分别通过；但post-fence故障journey在组合release gate的restart hydration处使Session catalog判为不可读，故MSG-020组合gate未通过且不得标记`certified`。S4的production-composed release gate已通过，accepted Interface与Adapter行为达到`conformant`；required完整lint仍有未修改基线blocker，因此暂不标记`certified`。S6的JSON Schema/submission语义与Message evidence rollback decisions已`accepted + conformant`；Codex、Claude Code、OpenCode production adapters与issued child capability使用同一Owner，desktop production-composed S6 journey已覆盖真实child经REPL提交、文本、Artifact、structured result及重启读取。完整release gate仍被既有Renderer focus restoration阻塞，所以不标`certified`。不得把S3及后续行为并入S2、S4、S5或S6。
...
| S5  | 可靠双向父子通信                   | Main与direct Subagent可双向发送、回复并恢复 queued/accepted/failed/uncertain receipt | `accepted`    | `conformant`   | `ReliableMessageDeliveryOwner`、Session persistence owner与两方runtime Adapters | `SUB-DEC-0003..0004`、`SUB-DEC-0008..0009` | `message-delivery.md`                                                   |
